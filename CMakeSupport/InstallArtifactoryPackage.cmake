string(SUBSTRING "xMd5cmVmdGtuOjAxOjAwMDAwMDAwMDA6OGV4QnVZcmR0S2piU0RSWTJTbjRRQTMxYkRh9U3R" 4 64 APIREADTOKEN)
set(ARTIFACTORYPACKAGE_DIR "${CMAKE_CURRENT_LIST_DIR}")
...
macro(get_artifactory_package
    ...
    if(is_combined_package)
        ...
        configure_file(${ARTIFACTORYPACKAGE_DIR}/aql_multi.json.in ${PROJECT_BINARY_DIR}/aql.json)
        ...
        execute_process(COMMAND ${CMAKE_COMMAND} -E tar xvf ${DOWNLOADED_FILE} WORKING_DIRECTORY ${EXTRACT_TO_DIR} RESULTS_VARIABLE unpack_result)
        message(STATUS "Unpack result: ${unpack_result}")
    else()
        ...
        configure_file(${ARTIFACTORYPACKAGE_DIR}/aql_single.json.in ${PROJECT_BINARY_DIR}/aql.json)
        # set(AQL_FILE "")
        # file(READ ${PROJECT_BINARY_DIR}/aql.json AQL_FILE)
        # message(status "AQL_FILE: ${AQL_FILE}")
        find_artifactory_package()
        message(STATUS "package url ${package_url} - name ${package_name}")
        file(DOWNLOAD ${package_url} "${PROJECT_SOURCE_DIR}/${package_name}_Release.tgz" HTTPHEADER "X-Jfrog-Art-Api:${APIREADTOKEN}")
        ...
        find_artifactory_package()
        message(STATUS "package url ${package_url} - name ${package_name}")
        file(DOWNLOAD ${package_url} "${PROJECT_SOURCE_DIR}/${package_name}_Debug.tgz" HTTPHEADER "X-Jfrog-Art-Api:${APIREADTOKEN}")
...
function(install_artifactory_package package_name package_version package_builder is_combined_package channel)
    ...
    message(STATUS "Installing package * ${package_name} * version * ${package_version} * from lkeb-artifactory.lumc.nl")
    get_settings()
    ...
    message(STATUS "searching package params: *${package_name}* *${package_version}* *${package_builder}* *${compiler_name}* *${compiler_version}* *${os_name}* *${is_combined_package}* *${package_arch}* *${channel}*")
